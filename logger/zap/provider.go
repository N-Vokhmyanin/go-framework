...
func (p *ZapProvider) WithConfig(cfg *zap.Config) *ZapProvider {
	p.config = cfg
	return p
}

func (p *ZapProvider) Config(c contracts.ConfigSet) {
	c.StringVar(&p.level, "LOG_LEVEL", "", "min log level")
	c.StringVar(&p.cfgFile, "LOG_CONFIG_FILE", "", "log config file")
}

func (p *ZapProvider) Boot(a contracts.Application) {
	a.Singleton(
...
func (p *ZapProvider) Register(contracts.Application) {
	// nothing
}

func (p *ZapProvider) Logger(_ contracts.Application) logger.Logger {
	return p.log
}

func (p *ZapProvider) getZapLogger(isDev bool, inCfg *zap.Config) *zap.Logger {
	if p.zap != nil {
		return p.zap
	}
...
func (p *ZapProvider) loadConfig() (*zap.Config, error) {
	if p.cfgFile == "" {
		return nil, nil
	}
	cfg := &config{}
